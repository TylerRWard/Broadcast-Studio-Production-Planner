...
tableActual.addEventListener('click', function(event) {
    ...
    //Stores the details in previousTypedData for (this) function
    ...
    console.log(`${previousTypedData.block}-${previousTypedData.item_num} Clicked column: ${columnName}, row: ${rowIndex}, rundown name: ${selectedRundown.show_name}, rundown date: ${selectedRundown.show_date}`);
...
tableActual.addEventListener('input', function(event) {
    ...
    else if((rowIndex === selectedScriptRow.row_number) && (columnName==='ITEM_NUM') && (input.value !==''))
    {
    ...
    {
        // if there's no block and item_num in row relevant to selected td
