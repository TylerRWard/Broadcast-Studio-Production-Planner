...
let selectedScriptRow = {
    row_num: null,         //it starts from 1, sice this is declaration I give it null
    block: null,
    item_num: null,
    words: ''
}
...
tableActual.addEventListener('click', function(event) {
    ...
    console.log(`Clicked column: ${columnName}, row: ${rowIndex}, rundown name: ${selectedRundown.show_name}, rundown date: ${selectedRundown.show_date}`);
...
tableActual.addEventListener('input', function(event) {
    ...
    if((columnName==='BLOCK') && (input.value !==''))
    {   
        selectedScriptRow.row_num = rowIndex;
        selectedScriptRow.block = input.value;
    }

    if((rowIndex === selectedScriptRow.row_num) && (columnName==='ITEM_NUM') && (input.value !==''))
    {
        selectedScriptRow.item_num = input.value;
        console.log("I have both block and item num in a same row", selectedScriptRow.block ,selectedScriptRow.item_num)
        ////if so create a row in slots_t2
        insertRowSlots_t();
    } 
});
...
async function insertRowSlots_t() {
    ...
    const data = {
        ...
        line_id: selectedScriptRow.row_num
    }

    console.log(JSON.stringify(data));

    try {
        const response = await fetch("http://localhost:3000/add-row-slots_t", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
