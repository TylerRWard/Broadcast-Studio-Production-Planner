...
app.post("/add-show", async (req, res) => {
  ...
  app.get("/print-rundown/:show_name/:show_date", isAuthenticated, async (req, res) => {
    const { show_name, show_date } = req.params;
    ...
    try {
        ...
        if (!scriptData.rows.length) {
            return res.status(404).json({ message: "No scripts found." });
        }

        res.json({
            scripts: scriptData.rows,
            show_name,
            show_date,
        });
    } catch (err) {
        console.error("Error fetching rundown:", err);
        res.status(500).json({ error: "Server error while fetching rundown." });
    }
});
